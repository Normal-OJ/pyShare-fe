<template>
  ...
  >
    ...
    <div class="d-flex align-center mt-4">
      <div class="text-body-1 mt-2 mr-2">
        上傳附件
      </div>
      <v-file-input
      ...
      >
        <template #append>
          <v-btn
            ...
            color="error"
            small
            @click.stop="removeAttachmentFromProb(att)"
          >
            移除
          </v-btn>
        </template>
      ...
      >
        <template #append>
          <v-btn
            ...
            color="error"
            small
            @click.stop="removeFromWillAddAttachments(att)"
          >
            移除
          </v-btn>
        </template>
      ...
      >
        <template #append>
          <v-btn
            ...
            color="error"
            small
            @click.stop="removeFromWillImportAttachments(att)"
          >
            移除
          </v-btn>
        </template>
    ...
    <div
      v-if="willRemoveAttachments.length > 0"
      class="d-flex align-center flex-wrap mt-1"
    >
      <v-tooltip bottom>
        <template #activator="{ on, attr }">
      ...
      >
        <template #append>
          <v-btn
            ...
            color="error"
            small
            @click.stop="undoRemoveAttachmentFromProb(att)"
          >
            還原
          </v-btn>
        </template>
...
export default {
  ...
  methods: {
    ...
    removeAttachmentFromProb(removedFile) {
      ...
      this.newProb.attachments = this.newProb.attachments.filter(
        (file) => file.filename !== removedFile.filename,
      )
    },
    undoRemoveAttachmentFromProb(undoFile) {
      ...
      this.newProb.attachments.push(undoFile)
    },
    goBack() {
